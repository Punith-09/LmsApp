...
//         router.push("/instructor/courses/courses");
//       } else {
...
//             onClick={() => router.push("/instructor/courses/courses")}
//           />
...
export default function CreateCourse() {
  ...
  return (
    ...
    >
      <Card style={{ margin: "1rem" }}>
        <Title level={3}>
          <LeftOutlined style={{ paddingRight: "15px" }} onClick={() => router.push("/instructor/courses/courses")} />
          {isEdit ? "Edit Course" : "Create New Course"}
        </Title>
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item label="Intro Video" required>
            ...
            {introVideo && <video width="300" controls style={{ marginTop: 10 }}><source src={introVideo} /></video>}
          </Form.Item>
              ...
              <>
                {secFields.map((secField) => (
                  <Card
                    ...
                    style={{ marginBottom: 20 }}
                  >
                    <Form.Item name={[secField.name, "title"]} label="Section Title" rules={[{ required: true }]}>
                      <Input placeholder="Section title" />
                    </Form.Item>
                    ...
                    <Form.List name={[secField.name, "lessons"]}>
                      {(lessonFields, { add: addLesson, remove: remLesson }) => (
                        <>
                          {lessonFields.map((lessonField) => (
                            <Card key={lessonField.key} size="small" style={{ marginBottom: 12 }}
                                  title={`Lesson ${lessonField.name + 1}`}
                                  extra={
                                    <Button danger type="link" icon={<MinusCircleOutlined />}
                                            onClick={() => remLesson(lessonField.name)}>Remove Lesson</Button>
                                  }>
                              <Form.Item name={[lessonField.name, "title"]} label="Lesson Title" rules={[{ required: true }]}>
                                <Input placeholder="Lesson title"/>
                              </Form.Item>
                              ...
                              <Form.Item name={[lessonField.name, "videoUrl"]} hidden rules={[{ required: true }]}>
                                <Input type="hidden"/>
                              </Form.Item>
                            </Card>
                          ))}
                          <Button type="dashed" onClick={() => addLesson()} block icon={<PlusCircleOutlined />}>Add Lesson</Button>
                        </>
                      )}
                    </Form.List>
                  </Card>
                ))}
                <Button type="dashed" onClick={() => addSec()} block icon={<PlusCircleOutlined />}>Add Section</Button>
              </>
            )}
          </Form.List>

          <Form.Item>
            <Button type="primary" htmlType="submit" loading={isLoading} style={{ width: "20%", marginTop: 15 }}>
              {isEdit ? "Update Course" : "Create Course"}
            </Button>
          </Form.Item>
        </Form>
